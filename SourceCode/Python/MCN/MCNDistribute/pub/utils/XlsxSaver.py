...
class XlsxSaver:
    ...
    def __init__(self, df_in, filename='a.xlsx', sheet_name='Sheet1'):
        ...
        if path.exists(filename):
            ...
            if is_sheet_exist(filename, sheet_name):
                sheetDel = self.wb[sheet_name]
                self.wb.remove_sheet(sheetDel)
    ...
    def set_color_alignment_font_border(self, color_header, color_0, color_1, header_size, content_size, font_name):
        ...
        # 标题行
        for col in range(0, len(list(self.df.columns))):
            header_cell = self.sheet.cell(0 + 1, col + 1)
            header_cell.fill = PatternFill(fill_type="solid", start_color=color_header,
                                                               end_color=color_header)  # 注意：sheet行列从1开始计数
            header_cell.font = Font(name=font_name, size=header_size, bold=True)  # 设置字体大小和加粗)
            header_cell.border = border
            header_cell.alignment = alignment
        for row in range(0, len(list(self.df.index))):
            for col in range(0, len(list(self.df.columns))):
                content_cell = self.sheet.cell(row + 2, col + 1)
                # 居中
                content_cell.alignment = alignment
                # 边框
                content_cell.border = border
                # 字体
                content_cell.font = Font(name=font_name, size=content_size, bold=True)  # 设置字体大小和加粗)
                # 隔行换色
                if row % 2 == 0:
                    content_cell.fill = PatternFill(fill_type="solid", start_color=color_0,
                                            end_color=color_0)  # 注意：sheet行列从1开始计数
                if color_1 is not None:
                    content_cell.fill = PatternFill(fill_type="solid", start_color=color_1,
                                            end_color=color_1)  # 注意：sheet行列从1开始计数


    def set_alignment_font_border(self, col_name):
        # 根据列名设置居中、字体、边框
        index = list(self.df.columns).index(col_name)
        letter = chr(index + 65)
        for cell in self.sheet[letter]:
            ...
            cell.border = Border(left=Side(style='thin'),
                                 ...
                                 top=Side(style='thin'),
                                 bottom=Side(style='thin'))
            cell.font = Font(size=11)  # 设置字体大小和加粗)
            # cell.font = Font(size=11, bold=True)  # 设置字体大小和加粗)

    # def set_line(self, col_name):
    #     index = list(self.df.columns).index(col_name)
    #     letter = chr(index + 65)
    #     for cell in self.sheet[letter]:
    #         cell.
