...
from pub.utils.ExcelFormat import set_alignment_font_border
...
def setMCNOrderExcel(fileName, sheetName, headerTitle):
    wb = pyxl.load_workbook(filename=fileName)
    ws = wb[sheetName]
    # 其他打印设置
    ws.page_setup.paperSize = "9"  # 纸张尺寸参见上表
    ...
    # 设置页眉 左中右 left center right
    ws.oddHeader.center.text = headerTitle  # 文本
    ws.oddHeader.center.size = 20  # 字号
    ws.oddHeader.center.font = "微软雅黑"  # 字体
    ws.oddHeader.center.color = "000000"  # 16进制RGB颜色 参照PS
    # 设置页脚
    ws.oddFooter.center.text = "第&[Page]页 共&[Pages]页"
    ...
    for cell in ws[1]:
        if cell.value == '采购数量':
            indexColumn = cell.column_letter
    indexColumn = str(indexColumn)
    # 最后添加一行的行序号
    indexRow = ws.max_row + 1
    # 行+列 的编号
    sumCellIndex = indexColumn + str(indexRow)
    # 设置合计

    ws[sumCellIndex] = "=SUBTOTAL(9, "+indexColumn+'2'+":"+indexColumn+str(ws.max_row)+")"

    set_alignment_font_border(ws[sumCellIndex], 'center', 15)

    # 直接计算ASCII值减一
    new_col = chr(ord(indexColumn) - 1)
    startIndex = 'A' + str(indexRow)
    endIndex = new_col+str(indexRow)
    # 设置合并单元格
    ws.merge_cells(startIndex + ':' + endIndex)
    ws['A'+str(indexRow)] = '合计数量'
    set_alignment_font_border(ws[startIndex], 'center', 18)
    # cell.column_letter

    # 设置边框
    for row in ws.iter_rows(min_row=ws.max_row, max_row=ws.max_row, min_col=1, max_col=ws.max_column):
        for cell in row:
            set_alignment_font_border(cell, 'center', 15)
